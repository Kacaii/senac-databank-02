...
WHERE
    pr.ativo = 1
    AND CURRENT_DATE BETWEEN pr.data_inicio AND COALESCE(
        pr.data_fim, CURRENT_DATE
    )
...
SELECT
    ...
    GROUP_CONCAT(p.nome, ', ') AS lista_produtos
...
CREATE VIEW vw_metodos_pagamento_utilizados AS
SELECT
    fp.id AS metodo_id,
...
CREATE VIEW vw_categorias_faturamento AS
SELECT
...
CREATE VIEW vw_historico_vendas_detalhado AS
SELECT
    v.id AS venda_id,
    v.data_venda,
    c.nome AS cliente,
    ...
    iv.quantidade,
    iv.preco_unitario,
    CASE WHEN iv.id_promocao IS NOT NULL THEN pr.nome ELSE 'Sem promoção' END
        AS promocao,
